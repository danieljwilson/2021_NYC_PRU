      ...
      tabItem(tabName = "poverty_about",
              ...
              fluidPage(
                ...
                box(
                  ...
                  tags$ul(
                    tags$li('A poverty measure poverty measure serves as a ', tags$strong('social indicator'), ' that gauges the degree of material deprivation in our society.'), 
              ...
              fluidPage(
                ...
                fluidRow(
                  box(
                    ...
                    p('There were no statistically significant increases in poverty or near poverty for any group from 2018 to 2019, or from 2015 to 2019. Detailed data tables for poverty rates are provided in Chapter 4.'),
                    h1('Figure 1.3: NYCgob Poverty Rates by Age, 2015-2019'),
                    h1('Figure 1.4: NYCgov Poverty Rates by Sex, 2015–2019'),
                    h1('Figure 1.5: NYCgov Poverty Rates by Race/Ethnicity, 2015–2019'),
                    h1('Figure 1.6: Poverty Rates, Race and Ethnicity by Gender, 2015–2019'),
                    h1('Figure 1.7: NYCgov Poverty Rates by Citizenship Status, 2015–2019'),
                    h1('Figure 1.8: NYCgov Poverty Rates by Educational Attainment, 2015–2019'),
                    h1('Figure 1.9: NYCgov Poverty Rates by Work Experience, 2015–2019'),
                    h1('Figure 1.10: NYCgov Poverty Rates by Borough, 2015–2019'),
                    h1('Figure 1.11: Percentage of Population Below Poverty Threshold, by Neighborhood, 2015–2019'),
                    h1('Table 1.2: Racial and Ethnic Composition of Community Districts (CDs) with Highest and Lowest Poverty Rates, 2015–2019')
                  )
                ),
                fluidRow(
                  box(
                ...
                fluidRow(
                  box(
                    ...
                    h2('Figure 1.17: Impact of Combined Government Assistance and Tax Credits by Selected Family Type, 2019 (Percent Decline in Poverty Rate)')
                  )
                ),
                fluidRow(
                  box(
                    ...
                    h2('Table 1.5: Comparison of U.S. Official and NYCgov Poverty Measures')
                  )
                )
                )
             ),
...
server <- function(input, output) {
  ...
  selections = reactive({
    color_cols <- function(df){
      for (i in 1:(length(names(df))-5)){
        # extract levels
        col_levels = unique(df[[names(df)[i]]])
        # length levels
        col_length = length(col_levels)
        # colors to match levels
        col_colors = color_palette[1:col_length]
        # format column
        formatStyle(names(df)[i],
                    backgroundColor = styleEqual(col_levels,
                                                 col_colors)
        )
      }
    }
    ...
    dt = dt %>%
      ...
      formatStyle('n_pov_weighted',
                  ...
                  backgroundRepeat = 'no-repeat',
                  backgroundPosition = 'center') %>%
      # add colors by level for each column
    ...
    for (i in 1:(length(names(dt))-5)){
      formatStyle(names(dt)[i],
                    backgroundColor = styleEqual(unique(dt[[names(dt)[i]]]),
                                                 color_palette[1:length(unique(dt[[names(dt)[i]]]))]
                                                 )
      )
    }
